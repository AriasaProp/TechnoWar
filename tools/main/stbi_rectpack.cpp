...
void stbi::rectpack::setup_allow_out_of_mem (stbi::rectpack::context *context, bool allow_out_of_mem) {
  if (allow_out_of_mem)
...
void stbi::rectpack::init_target (stbi::rectpack::context *context, int width, int height, stbi::rectpack::node *nodes, int num_nodes) {
  ...
  stbi::rectpack::setup_allow_out_of_mem (context, false);
...
static int STBRP__CDECL rect_size_compare (const void *a, const void *b) {
  const stbi::rectpack::rect *p = (const stbi::rectpack::rect *)a;
  const stbi::rectpack::rect *q = (const stbi::rectpack::rect *)b;
  int ps = p->w * p->h, qs = q->w * q->h;
  return (ps > qs) ? -1 : (ps < qs);
}
...
bool stbi::rectpack::pack_rects (stbi::rectpack::context *context, stbi::rectpack::rect *rects, int num_rects) {
  ...
  qsort (rects, num_rects, sizeof (stbi::rectpack::rect), rect_size_compare);
