...
static void stbtt__rasterize_sorted_edges(stbtt__bitmap *result, stbtt__edge *e, int n, int vsubsample, int off_x, int off_y, void *userdata) {
  ...
  while (j < result->h) {
    ...
    memset(scanline, 0, (result->w * 2 + 1) * sizeof(float));
...
static void stbtt__rasterize(stbtt__bitmap *result, vec2 *pts, int *wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void *userdata) {
...
void stbtt_Rasterize(stbtt__bitmap *result, float flatness_in_pixels, stbtt_vertex *vertices, int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, int invert, void *userdata) {
  ...
  vec2 *windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, &winding_lengths, &winding_count, userdata);
  if (windings) {
...
unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, float scale_x, float scale_y, float shift_x, float shift_y, int glyph, int *width, int *height, int *xoff, int *yoff) {
  ...
  stbtt_GetGlyphBitmapBoxSubpixel(info, glyph, (vec2){scale_x, scale_y}, (vec2){shift_x, shift_y}, &ip0, &ip1);
  // now we get the size
  gbm.w = (ip1.x - ip0.x);
  gbm.h = (ip1.y - ip0.y);
  ...
  if (gbm.w && gbm.h) {
    ...
    if (gbm.pixels) {
      gbm.stride = gbm.w;
      stbtt_Rasterize(&gbm, 0.35f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ip0.x, ip0.y, 1, info->userdata);
    }
  }
  free(vertices);
  return gbm.pixels;
...
void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph) {
  ...
  int num_verts = stbtt_GetGlyphShape(info, glyph, &vertices);
  stbtt__bitmap gbm;
  stbtt_GetGlyphBitmapBoxSubpixel(info, glyph, (vec2){scale_x, scale_y}, (vec2){shift_x, shift_y}, &ip0, NULL);
  gbm.pixels = output;
  gbm.w = out_w;
  gbm.h = out_h;
  gbm.stride = out_stride;
  if (gbm.w && gbm.h) stbtt_Rasterize(&gbm, 0.35f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ip0.x, ip0.y, 1, info->userdata);
  free(vertices);
}
...
unsigned char *stbtt_GetGlyphSDF(const stbtt_fontinfo *info, float scl, int glyph, int padding, unsigned char onedge_value, float pixel_dist_scale, int *width, int *height, ivec2 *off) {
  if (!scl) return NULL;
  ...
  scale.y *= -1;
  {
    ...
    for (y = ip0.y; y < ip1.y; ++y) {
      for (x = ip0.x; x < ip1.x; ++x) {
        ...
        val = CLAMP(0, val, 255);
        data[(y - ip0.y) * w + (x - ip0.x)] = (unsigned char)val;
      }
    }
    free(precompute);
    free(verts);
...
unsigned char *stbtt_GetCodepointSDF(const stbtt_fontinfo *info, float scale, int codepoint, int padding, unsigned char onedge_value, float pixel_dist_scale, int *width, int *height, ivec2 *off) {
  return stbtt_GetGlyphSDF(info, scale, stbtt_FindGlyphIndex(info, codepoint), padding, onedge_value, pixel_dist_scale, width, height, off);
}
...
int stbtt_BakeFontBitmap(const unsigned char *data, int offset, float pixel_height, unsigned char *pixels, int pw, int ph, int first_char, int num_chars, stbtt_bakedchar *chardata) {
  ...
  for (i = 0; i < num_chars; ++i) {
    ...
    ASSERT(x + gw < pw);
    ASSERT(y + gh < ph);
    stbtt_MakeGlyphBitmap(&f, pixels + x + y * pw, gw, gh, pw, scale.x, scale.y, g);
