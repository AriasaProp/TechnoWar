...
static int assetBuffer(const char *filename, void **buf, int *len)
{
  for (size_t i = 0; i < MAX_ASSET_READING; ++i) {
    if (!reading[i]) {
      reading[i] = AAssetManager_open(assetManager, filename, AASSET_MODE_BUFFER);
...
static int openAsset(const char *filename)
{
  for (size_t i = 0; i < MAX_ASSET_READING; ++i) {
    if (!reading[i]) {
      reading[i] = AAssetManager_open(assetManager, filename, AASSET_MODE_STREAMING);
      return i;
    }
  }
  return -1;
