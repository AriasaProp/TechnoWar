...
void uistage::loadUISkin (const char *uiSkin) {
  {
    ...
    while (!buffer_stream.eof ()) {
      ...
      uiskin::regions.insert ({name, {.pos = {(unsigned int)std::stoul (npos1), (unsigned int)std::stoul (npos2)}, .size = {(unsigned int)std::stoul (nsize1), (unsigned int)std::stoul (nsize2)}, .patch = {0}, .rc = {.color = 0xffffffff}}});
    }
  }
