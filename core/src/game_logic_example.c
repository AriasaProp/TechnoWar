...
void game_init() {
  ...
  for (int i = 0; i < MAX_BOX; ++i) {
    vel[i].x = (2f * rands()) - 1f;
    vel[i].y = (2f * rands()) - 1f;
    float x = 2800f * rands(), y = 720f * rands();
    rects[i * 4 + 0] = (vec2){x + 25f, y + 25f}; // Bottom-right
    rects[i * 4 + 1] = (vec2){x + 25f, y - 25f}; // Top-right
    rects[i * 4 + 2] = (vec2){x - 25f, y + 25f}; // Bottom-left
    rects[i * 4 + 3] = (vec2){x - 25f, y - 25f}; // Top-left
  }
}
struct flat_vertex *game_update(unsigned int *l) {
  ...
  for (int i = 0; i < MAX_BOX; ++i) {
    ...
    if ((rects[i * 4 + 3].pos.y < -50) ||
        (rects[i * 4 + 3].pos.y > 700)) {
      vel[i].y *= -1f;
    }
    if ((rects[i * 4 + 3].pos.x < -100) ||
        (rects[i * 4 + 3].pos.x > 3200)) {
      vel[i].x *= -1f;
    }
  }
  return rects;
